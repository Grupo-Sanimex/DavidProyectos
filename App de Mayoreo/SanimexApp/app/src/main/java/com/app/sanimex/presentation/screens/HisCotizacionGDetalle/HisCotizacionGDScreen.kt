...
@Composable
fun HistCotizacionGDScreen(
    fecha : String,
    idVistador : String,
    onNavigationBack: () -> Unit,
    onNavigateToDetail: (String,String,String) -> Unit,
    viewModel:HisCotizacionGDViewModel = hiltViewModel()
){
    val uiState by viewModel.uiState.collectAsState()
    LaunchedEffect(fecha, idVistador) {
        viewModel.verifieDate(fecha, idVistador)
    }
    Scaffold( topBar = {
        TopAppBar(
            title = { Text(text = stringResource(R.string.HistCotizacionG),
                    ...
                    LazyColumn(
                        ...
                        items(uiState.hisCotizaciones) { cotizacion ->
                            CotizacionCard(
                                ...
                                onCotizacionClick = {
                                    try {
                                        onNavigateToDetail(cotizacion.idCotizacion, fecha, idVistador)
                                    } catch (e: Exception) {
                                    }
                                }
                            )
